...
describe("rowsToLeaderboardCsv", () => {
  it("writes the exact fixed header plus per-question value/probabilities/confidence columns", () => {
    ...
    expect(csv).toBe(
      ...
      "clarity_what_is_it_value,clarity_what_is_it_probabilities,clarity_what_is_it_confidence," +
      "pricing_visibility_value,pricing_visibility_probabilities,pricing_visibility_confidence," +
      "overall,usage_input_tokens,usage_output_tokens,usage_total_tokens",
    );
  });

  it("flattens an ok row with answers, probabilities, confidence, and usage", () => {
    const row: LeaderboardRow = {
      ...
      total_ms: 950,
      answers: {
        clarity_what_is_it: { type: "score", value: 3, probabilities: { "3": 0.6, "4": 0.4 }, confidence: 0.82 },
    ...
    expect(lines[1]).toBe(
      ...
      '3,"{""3"":0.6,""4"":0.4}",0.82,visible_price,,,' +
      '75,1000,200,1200',
    );
  });
